...
# from DAXXROBOT.modules.sql import warns_sql as warnssql
import DAXXROBOT.modules.sql.blacklist_sql as blacklistsql

# from DAXXROBOT.modules.sql import cust_filters_sql as filtersql
# import DAXXROBOT.modules.sql.welcome_sql as welcsql
import DAXXROBOT.modules.sql.locks_sql as locksql
import DAXXROBOT.modules.sql.notes_sql as sql

# from DAXXROBOT.modules.rules import get_rules
import DAXXROBOT.modules.sql.rules_sql as rulessql
from DAXXROBOT import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from DAXXROBOT.__main__ import DATA_IMPORT
from DAXXROBOT.modules.connection import connected
from DAXXROBOT.modules.helper_funcs.alternate import typing_action
from DAXXROBOT.modules.helper_funcs.chat_status import user_admin
from DAXXROBOT.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open(f"DAXXROBOT{chat_id}.backup", "w") as f:
        f.write(baccinfo)
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("DAXXROBOT{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `DAXXROBOT-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("DAXXROBOT{}.backup".format(chat_id))  # Cleaning file
