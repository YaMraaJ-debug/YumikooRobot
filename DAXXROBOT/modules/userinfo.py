...
from DAXXROBOT.modules.alive import Mukesh
import DAXXROBOT.modules.sql.userinfo_sql as sql
...
from DAXXROBOT.__main__ import STATS, TOKEN, USER_INFO
from DAXXROBOT.modules.disable import DisableAbleCommandHandler
from DAXXROBOT.modules.helper_funcs.chat_status import sudo_plus
from DAXXROBOT.modules.helper_funcs.extraction import extract_user
from DAXXROBOT.modules.sql.global_bans_sql import is_user_gbanned
from DAXXROBOT.modules.sql.users_sql import get_user_num_chats
...
def info(update: Update, context: CallbackContext):
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://t.me/HEROKUFREECC">ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴡʜᴀᴛ ɪs ᴅɪsᴀsᴛᴇʀ ʟᴇᴠᴇʟs.</a>]'.format(
            bot.username
        )
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            ...
                            InlineKeyboardButton(
                                "Dɪsᴀsᴛᴇʀ", url="https://t.me/HEROKUFREECC"
                            ),
                        ],
                        [
                            InlineKeyboardButton(
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if len(info) == 2:
        if len(info[1]) < MAX_MESSAGE_LENGTH // 4:
            ...
            message.reply_text(
                f"ᴛʜᴇ ɪɴғᴏ ɴᴇᴇᴅs ᴛᴏ ʙᴇ ᴜɴᴅᴇʀ {MAX_MESSAGE_LENGTH // 4} ᴄʜᴀʀᴀᴄᴛᴇʀs! ʏᴏᴜ ʜᴀᴠᴇ {len(info[1])}."
            )
    ...
    if message.reply_to_message:
        ...
        if len(bio) == 2:
            if len(bio[1]) < MAX_MESSAGE_LENGTH // 4:
                ...
                message.reply_text(f"ᴜᴘᴅᴀᴛᴇᴅ {repl_message.from_user.first_name}'s ʙɪᴏ!")
            else:
                message.reply_text(
